...
class StockProfile():
    ...
    def update(self) -> dict:
        """
        Updates outdated profiles. 
        Returns a dict with successfully updated companies, and error messages.
        """
        ...
        for company in self.companies:
            resp = self.request(company.symbol)

            if not resp.ok:
                error = self.log_error(company, resp)
                companies["errors"].append(error)
                continue

            data = resp.json()['quoteSummary']['result'][0]['assetProfile']

            self.update_website(company, data)
            self.update_summary(company, data)
            self.update_sector(company, data)
            self.update_location(company, data)
            company.profile_last_retrieved = self.datetime_now

            companies['updated'].append(company)
